...
async function resolveVendorId(userId) {
    if (userId.startsWith('dev-'))
        return null;
    const result = await query(`SELECT id FROM vendors WHERE owner_user_id=$1 LIMIT 1`, [userId]);
    return result.rows[0]?.id ?? null;
}
router.get('/me', async (req, res, next) => {
    try {
        const vendorId = await resolveVendorId(req.authUser.id);
        if (!vendorId) {
...
router.get('/dashboard', async (req, res, next) => {
    try {
        ...
        const [orders, sales, low, v] = await Promise.all([
            query(`SELECT COUNT(*)::text count FROM orders WHERE vendor_id=$1 AND created_at >= CURRENT_DATE`, [vendorId]),
            query(`SELECT COALESCE(SUM(total_paise),0)::text sum FROM orders WHERE vendor_id=$1 AND status NOT IN ('CANCELLED','REFUNDED') AND created_at >= CURRENT_DATE`, [vendorId]),
            query(`SELECT COUNT(*)::text count FROM products WHERE vendor_id=$1 AND stock_qty<10 AND is_active=true`, [vendorId]),
            query('SELECT rating,review_count FROM vendors WHERE id=$1', [vendorId])
        ]);
...
router.post('/products', requirePermission('VIEW_OWN_SCOPE'), async (req, res, next) => {
    try {
        ...
        await audit('VENDOR_CREATE_PRODUCT', 'products', product.id, req.authUser.id, { vendorId });
...
router.patch('/products/:id', requirePermission('VIEW_OWN_SCOPE'), async (req, res, next) => {
    try {
        ...
        const current = await query(`SELECT * FROM products WHERE id=$1 AND vendor_id=$2`, [id, vendorId]);
        if (!current.rows[0]) {
            res.status(404).json({ error: 'PRODUCT_NOT_FOUND' });
            return;
        }
        const currentProduct = current.rows[0];
        const merged = { ...currentProduct, ...input };
        const r = await query(`UPDATE products SET category_id=$1,name=$2,description=$3,unit_label=$4,price_paise=$5,stock_qty=$6,image_url=$7,is_active=$8,updated_at=NOW() WHERE id=$9 AND vendor_id=$10 RETURNING *`, [merged.categoryId ?? currentProduct.category_id, merged.name, merged.description ?? null, merged.unitLabel ?? currentProduct.unit_label, Math.round((input.priceRupees ?? Number(currentProduct.price_paise) / 100) * 100), merged.stockQty ?? currentProduct.stock_qty, merged.imageUrl ?? currentProduct.image_url, merged.isActive ?? currentProduct.is_active, id, vendorId]);
        await audit('VENDOR_UPDATE_PRODUCT', 'products', id, req.authUser.id, { vendorId });
...
router.get('/orders/:id/pickup-code', async (req, res, next) => {
    try {
        const id = z.string().uuid().parse(req.params.id);
        ...
        const row = await query(`SELECT da.id AS assignment_id,o.status::text AS order_status FROM delivery_assignments da JOIN orders o ON o.id=da.order_id WHERE da.order_id=$1 AND o.vendor_id=$2 LIMIT 1`, [id, vendorId]);
        if (!row.rows[0]) {
...
router.get('/orders', async (req, res, next) => {
    try {
        ...
        const r = await query(`SELECT id,status,total_paise,created_at FROM orders WHERE vendor_id=$1 ORDER BY created_at DESC LIMIT 100`, [vendorId]);
...
const transitions = { PLACED: ['VENDOR_ACCEPTED', 'CANCELLED'], PAID: ['VENDOR_ACCEPTED', 'CANCELLED'], COD_CONFIRMED: ['VENDOR_ACCEPTED', 'CANCELLED'], VENDOR_ACCEPTED: ['PREPARING', 'CANCELLED'], PREPARING: ['READY_FOR_PICKUP', 'CANCELLED'] };
router.post('/orders/:id/status', requirePermission('MANAGE_OWN_ORDERS'), async (req, res, next) => {
    try {
        const id = z.string().uuid().parse(req.params.id);
        ...
        const current = await query(`SELECT status::text FROM orders WHERE id=$1 AND vendor_id=$2`, [id, vendorId]);
        if (!current.rows[0]) {
            res.status(404).json({ error: 'ORDER_NOT_FOUND' });
            return;
        }
        if (!(transitions[current.rows[0].status] ?? []).includes(status)) {
            res.status(409).json({ error: 'INVALID_TRANSITION', from: current.rows[0].status, to: status });
            return;
        }
        ...
        try {
            ...
            if (status === 'READY_FOR_PICKUP') {
                try {
                    await assignNearestDeliveryPartner(id);
                }
                catch (e) {
                    console.error('dispatch failed', e);
                }
            }
            await audit('VENDOR_ORDER_STATUS', 'orders', id, req.authUser.id, { vendorId, status });
...
router.get('/subscription', async (req, res, next) => {
    try {
        ...
        const r = await query(`SELECT vs.id,vs.status,vs.starts_at,vs.renews_at,vp.code,vp.name,vp.monthly_price_paise FROM vendor_subscriptions vs JOIN vendor_plans vp ON vp.id=vs.plan_id WHERE vs.vendor_id=$1 ORDER BY vs.created_at DESC LIMIT 1`, [vendorId]);
