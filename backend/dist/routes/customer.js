...
async function validateTargetedPromotion(client, promotion, userId, vendorId) {
    if (promotion.vendor_id && promotion.vendor_id !== vendorId)
        return 'PROMOTION_VENDOR_MISMATCH';
    ...
    if (Number(promotion.min_orders || 0) > orders)
        return 'PROMOTION_ORDER_HISTORY_REQUIREMENT';
    if (Number(promotion.min_spend_paise || 0) > spend)
        return 'PROMOTION_SPEND_REQUIREMENT';
    if (promotion.audience === 'NEW_CUSTOMER' && orders > 0)
        return 'PROMOTION_NEW_CUSTOMER_ONLY';
    if (promotion.audience === 'REPEAT_CUSTOMER' && orders < 2)
        return 'PROMOTION_REPEAT_CUSTOMER_ONLY';
    if (promotion.audience === 'MEMBERS' && !(await client.query(`SELECT 1 FROM customer_memberships WHERE user_id=$1 AND status='ACTIVE' LIMIT 1`, [userId])).rows[0])
        return 'PROMOTION_MEMBERS_ONLY';
    if (promotion.membership_required && !(await client.query(`SELECT 1 FROM customer_memberships WHERE user_id=$1 AND status='ACTIVE' LIMIT 1`, [userId])).rows[0])
        return 'PROMOTION_MEMBERSHIP_REQUIRED';
    if (Number(promotion.loyalty_min_points || 0) > 0) {
        ...
        if (Number(loyalty.rows[0]?.points ?? 0) < Number(promotion.loyalty_min_points))
            return 'PROMOTION_LOYALTY_REQUIREMENT';
    }
    if (promotion.category_id) {
        ...
        if (!match.rows[0])
            return 'PROMOTION_CATEGORY_MISMATCH';
    }
    if (promotion.service_zone_id) {
        ...
        if (!match.rows[0])
            return 'PROMOTION_ZONE_MISMATCH';
    }
    return null;
}
router.post('/serviceability', async (req, res, next) => {
    try {
        ...
        const zones = await query('SELECT id,city,name,radius_m,center_latitude,center_longitude FROM service_zones WHERE is_active=true');
...
router.get('/vendors', async (req, res, next) => {
    ...
    ORDER BY v.rating DESC,v.review_count DESC LIMIT $2`, [q.q ?? null, q.limit]);
        ...
        if (q.latitude != null && q.longitude != null) {
            vendors = vendors.map((v) => ({ ...v, distance_km: v.latitude != null && v.longitude != null ? haversineKm(q.latitude, q.longitude, Number(v.latitude), Number(v.longitude)) : null })).sort((a, b) => (a.distance_km ?? 999) - (b.distance_km ?? 999));
        }
...
router.get('/cart', async (req, res, next) => {
    ...
    LEFT JOIN cart_items ci ON ci.cart_id=cc.id LEFT JOIN products p ON p.id=ci.product_id WHERE cc.customer_id=$1 ORDER BY p.name`, [req.authUser.id]);
        res.json({ cart: r.rows[0] ? { cartId: r.rows[0].cart_id, vendorId: r.rows[0].vendor_id, items: r.rows.map((x) => ({ productId: x.product_id, name: x.name, pricePaise: Number(x.price_paise), imageUrl: x.image_url, quantity: x.quantity })) } : { cartId: null, vendorId: null, items: [] }, mode: 'database' });
    }
    catch (e) {
        next(e);
    }
...
router.put('/cart', async (req, res, next) => {
    ...
    try {
        ...
        const cart = await client.query('INSERT INTO customer_carts(customer_id,vendor_id) VALUES($1,$2) ON CONFLICT(customer_id) DO UPDATE SET vendor_id=EXCLUDED.vendor_id,updated_at=NOW() RETURNING id', [req.authUser.id, input.vendorId]);
        const cartId = cart.rows[0].id;
        await client.query('DELETE FROM cart_items WHERE cart_id=$1', [cartId]);
        for (const item of input.items) {
            await client.query('INSERT INTO cart_items(cart_id,product_id,quantity) VALUES($1,$2,$3)', [cartId, item.productId, item.quantity]);
        }
        await client.query('COMMIT');
        res.json({ saved: true, cartId });
    }
    catch (e) {
...
router.post('/addresses', async (req, res, next) => {
    ...
    try {
        ...
        await client.query('COMMIT');
        const address = r.rows[0];
        await audit('CUSTOMER_ADDRESS_CREATED', 'addresses', address.id, userId, { isDefault: input.isDefault });
...
router.patch('/addresses/:id', async (req, res, next) => {
    ...
    try {
        ...
        const userId = req.authUser.id;
        await client.query('BEGIN');
        const current = await client.query('SELECT * FROM addresses WHERE id=$1 AND user_id=$2', [id, userId]);
        if (!current.rows[0]) {
        ...
        await client.query('COMMIT');
        await audit('CUSTOMER_ADDRESS_UPDATED', 'addresses', id, userId, { fields: Object.keys(input) });
...
router.delete('/addresses/:id', async (req, res, next) => {
    try {
        const id = z.string().uuid().parse(req.params.id);
        const userId = req.authUser.id;
        const r = await query('DELETE FROM addresses WHERE id=$1 AND user_id=$2 RETURNING id,is_default', [id, userId]);
        if (!r.rows[0]) {
            res.status(404).json({ error: 'ADDRESS_NOT_FOUND' });
            return;
        }
        ...
        await audit('CUSTOMER_ADDRESS_DELETED', 'addresses', id, userId, {});
        res.status(204).send();
    }
    catch (e) {
        next(e);
...
router.patch('/notification-preferences', async (req, res, next) => {
    try {
        ...
        const params = [];
        let i = 1;
        const map = { orderUpdates: 'order_updates', deliveryUpdates: 'delivery_updates', paymentUpdates: 'payment_updates', supportUpdates: 'support_updates', promotions: 'promotions', productOffers: 'product_offers', pushEnabled: 'push_enabled', whatsappEnabled: 'whatsapp_enabled', smsEnabled: 'sms_enabled', emailEnabled: 'email_enabled' };
        ...
        if (fields.length) {
            params.push(userId);
            await query(`UPDATE customer_notification_preferences SET ${fields.join(',')},updated_at=NOW() WHERE user_id=$${i}`, params);
        }
...
router.get('/personalized-feed', async (req, res, next) => {
    try {
...
router.post('/orders/:id/reorder', async (req, res, next) => {
    ...
    try {
        ...
        await client.query('BEGIN');
        const source = await client.query(`SELECT id,vendor_id,status::text FROM orders WHERE id=$1 AND customer_id=$2 FOR SHARE`, [orderId, req.authUser.id]);
        if (!source.rows[0]) {
            await client.query('ROLLBACK');
            res.status(404).json({ error: 'ORDER_NOT_FOUND' });
            return;
        }
        if (source.rows[0].status !== 'DELIVERED') {
            await client.query('ROLLBACK');
            res.status(409).json({ error: 'REORDER_REQUIRES_DELIVERED_ORDER' });
            return;
        }
        const existing = await client.query(`SELECT vendor_id FROM customer_carts WHERE customer_id=$1`, [req.authUser.id]);
        if (existing.rows[0]?.vendor_id && existing.rows[0].vendor_id !== source.rows[0].vendor_id) {
            await client.query('ROLLBACK');
            res.status(409).json({ error: 'CART_HAS_ANOTHER_VENDOR', vendorId: existing.rows[0].vendor_id });
            return;
        }
    ...
    WHERE oi.order_id=$1 ORDER BY p.name`, [orderId]);
        ...
        if (!available.length) {
            await client.query('ROLLBACK');
            res.status(409).json({ error: 'REORDER_PRODUCTS_UNAVAILABLE' });
            return;
        }
        const cart = await client.query(`INSERT INTO customer_carts(customer_id,vendor_id) VALUES($1,$2) ON CONFLICT(customer_id) DO UPDATE SET vendor_id=EXCLUDED.vendor_id,updated_at=NOW() RETURNING id`, [req.authUser.id, source.rows[0].vendor_id]);
        const cartId = cart.rows[0].id;
        const skipped = [];
        const added = [];
        for (const item of items.rows) {
            if (!item.is_active || Number(item.stock_qty) <= 0) {
...
router.get('/orders/:id/tracking', async (req, res, next) => {
    try {
        const orderId = z.string().uuid().parse(req.params.id);
...
router.get('/orders/:id/delivery-code', async (req, res, next) => {
    try {
        const orderId = z.string().uuid().parse(req.params.id);
        const row = await query(`SELECT da.id AS assignment_id,da.status FROM delivery_assignments da JOIN orders o ON o.id=da.order_id WHERE da.order_id=$1 AND o.customer_id=$2 LIMIT 1`, [orderId, req.authUser.id]);
        if (!row.rows[0]) {
            res.status(404).json({ error: 'DELIVERY_NOT_ASSIGNED' });
            return;
        }
        const code = await revealDeliveryOtp(row.rows[0].assignment_id, 'DROP');
        if (!code) {
            res.status(409).json({ error: 'DELIVERY_CODE_NOT_READY' });
            return;
        }
...
router.get('/orders/:id/timeline', async (req, res, next) => {
    try {
        const orderId = z.string().uuid().parse(req.params.id);
        const owner = await query('SELECT id FROM orders WHERE id=$1 AND customer_id=$2 LIMIT 1', [orderId, req.authUser.id]);
        if (!owner.rows[0]) {
            res.status(404).json({ error: 'ORDER_NOT_FOUND' });
            return;
        }
...
router.post('/orders/:id/cancel', async (req, res, next) => {
    ...
    try {
        ...
        await client.query('BEGIN');
        const order = await client.query(`SELECT o.id,o.status::text,p.status::text AS payment_status,o.total_paise,o.wallet_applied_paise FROM orders o LEFT JOIN LATERAL (SELECT status FROM payments WHERE order_id=o.id ORDER BY created_at DESC LIMIT 1) p ON TRUE WHERE o.id=$1 AND o.customer_id=$2 FOR UPDATE`, [orderId, req.authUser.id]);
        ...
        if (!cancellable) {
            await client.query('ROLLBACK');
            res.status(409).json({ error: 'ORDER_NOT_CANCELLABLE', status: row.status });
            return;
        }
        ...
        if (needsWalletRefund)
            await creditWalletForRefund(client, req.authUser.id, walletRefund, orderId);
        if (needsProviderRefund)
            await client.query(`INSERT INTO refunds(order_id,amount_paise,status,reason,created_by) VALUES($1,$2,'PENDING',$3,$4) ON CONFLICT DO NOTHING`, [orderId, cashfreeRefund, reason, req.authUser.id]);
        if (!needsProviderRefund && needsWalletRefund)
...
router.post('/checkout/quote', async (req, res, next) => {
    ...
    try {
        ...
        if (!vendor.rows[0]) {
            await client.query('ROLLBACK');
            res.status(400).json({ error: 'VENDOR_NOT_AVAILABLE' });
            return;
        }
        ...
        if (items.rows.length === 0) {
            await client.query('ROLLBACK');
            res.status(400).json({ error: 'CART_EMPTY' });
            return;
        }
        const subtotal = items.rows.reduce((s, x) => s + Number(x.price_paise) * Number(x.quantity), 0);
        let discount = 0;
        let promotion = null;
        if (input.promotionCode) {
            ...
            if (!promotion.is_active || new Date(promotion.starts_at) > new Date() || (promotion.ends_at && new Date(promotion.ends_at) <= new Date())) {
                await client.query('ROLLBACK');
                res.status(409).json({ error: 'PROMOTION_INACTIVE' });
                return;
            }
            if (promotion.usage_limit != null && Number(promotion.total_redemptions) >= Number(promotion.usage_limit)) {
                await client.query('ROLLBACK');
                res.status(409).json({ error: 'PROMOTION_EXHAUSTED' });
                return;
            }
            if (Number(promotion.customer_redemptions) >= Number(promotion.per_customer_limit)) {
                await client.query('ROLLBACK');
                res.status(409).json({ error: 'PROMOTION_CUSTOMER_LIMIT' });
                return;
            }
            if (subtotal < Number(promotion.min_order_paise)) {
                await client.query('ROLLBACK');
                res.status(409).json({ error: 'PROMOTION_MIN_ORDER', minimumPaise: Number(promotion.min_order_paise) });
                return;
            }
            const targetError = await validateTargetedPromotion(client, promotion, req.authUser.id, input.vendorId);
            if (targetError) {
                await client.query('ROLLBACK');
                res.status(409).json({ error: targetError });
                return;
            }
        ...
        const membership = await client.query(`SELECT cm.id,mp.benefits FROM customer_memberships cm JOIN membership_plans mp ON mp.id=cm.plan_id WHERE cm.user_id=$1 AND cm.status='ACTIVE' ORDER BY cm.created_at DESC LIMIT 1`, [req.authUser.id]);
        ...
        const walletRow = await client.query(`SELECT balance_paise,is_active FROM wallet_accounts WHERE user_id=$1 FOR UPDATE`, [req.authUser.id]);
...
router.post('/orders', async (req, res, next) => {
    const client = await pool.connect();
    try {
        const input = orderSchema.parse({ ...req.body, idempotencyKey: req.body?.idempotencyKey ?? req.header('Idempotency-Key') });
        await client.query('BEGIN');
        if (input.idempotencyKey) {
            const existing = await client.query('SELECT id,status,total_paise,discount_paise,wallet_applied_paise FROM orders WHERE idempotency_key=$1 LIMIT 1', [input.idempotencyKey]);
            if (existing.rows[0]) {
                await client.query('COMMIT');
                res.json({ order: existing.rows[0], reused: true });
                return;
            }
        }
        const address = await client.query('SELECT id FROM addresses WHERE id=$1 AND user_id=$2', [input.addressId, req.authUser.id]);
        ...
        for (const item of items.rows) {
            if (Number(item.quantity) > Number(item.stock_qty)) {
                res.status(409).json({ error: 'INSUFFICIENT_STOCK', productId: item.id });
                await client.query('ROLLBACK');
                return;
            }
        }
        ...
        if (walletApplied > 0)
            await debitWalletForOrder(client, req.authUser.id, walletApplied, orderId);
        ...
        if (status === 'COD_CONFIRMED' || status === 'PAID')
            await postPlatformRevenueWithClient(client, orderId);
        if (promotion)
            await client.query(`INSERT INTO promotion_redemptions(promotion_id,user_id,order_id,discount_paise) VALUES($1,$2,$3,$4)`, [promotion.id, req.authUser.id, orderId, discount]);
