...
router.patch('/shopping-lists/:id', async (req, res, next) => {
    try {
...
router.delete('/shopping-lists/:id', async (req, res, next) => {
    try {
        const id = idSchema.parse(req.params.id);
...
router.post('/shopping-lists/:id/items', async (req, res, next) => {
    try {
        const listId = idSchema.parse(req.params.id);
...
router.delete('/shopping-lists/:id/items/:productId', async (req, res, next) => {
    try {
        const listId = idSchema.parse(req.params.id);
        const productId = idSchema.parse(req.params.productId);
        const owns = await query('SELECT id FROM shopping_lists WHERE id=$1 AND user_id=$2 LIMIT 1', [listId, req.authUser.id]);
...
router.post('/reviews', async (req, res, next) => {
    ...
    try {
        ...
        const eligible = await client.query(`SELECT o.vendor_id FROM orders o JOIN order_items oi ON oi.order_id=o.id AND oi.product_id=$2
    WHERE o.id=$1 AND o.customer_id=$3 AND o.status='DELIVERED' LIMIT 1`, [input.orderId, input.productId, userId]);
        if (!eligible.rows[0]) {
            await client.query('ROLLBACK');
            res.status(409).json({ error: 'REVIEW_NOT_ELIGIBLE' });
            return;
        }
...
router.post('/security/revoke-other-sessions', async (req, res, next) => {
    try {
...
router.get('/saved-payments', async (req, res, next) => {
    try {
...
router.post('/gift-cards/redeem', async (req, res, next) => {
    ...
    try {
        ...
        if (duplicate.rows[0]) {
            await client.query('COMMIT');
            res.json({ redeemed: true, idempotent: true });
            return;
        }
        const card = await client.query(`SELECT id,balance_paise,status,expires_at FROM gift_cards WHERE code_hash=$1 FOR UPDATE`, [codeHash]);
        const row = card.rows[0];
        if (!row) {
            await client.query('ROLLBACK');
            res.status(404).json({ error: 'GIFT_CARD_NOT_FOUND' });
            return;
        }
        if (row.status !== 'ACTIVE') {
            await client.query('ROLLBACK');
            res.status(409).json({ error: 'GIFT_CARD_NOT_ACTIVE' });
            return;
        }
        if (row.expires_at && new Date(row.expires_at).getTime() <= Date.now()) {
            await client.query(`UPDATE gift_cards SET status='EXPIRED',updated_at=NOW() WHERE id=$1`, [row.id]);
            await client.query('COMMIT');
            res.status(409).json({ error: 'GIFT_CARD_EXPIRED' });
            return;
        }
        const amount = Number(row.balance_paise);
        if (amount <= 0) {
            await client.query(`UPDATE gift_cards SET status='REDEEMED',updated_at=NOW() WHERE id=$1`, [row.id]);
            await client.query('COMMIT');
            res.status(409).json({ error: 'GIFT_CARD_EMPTY' });
            return;
        }
