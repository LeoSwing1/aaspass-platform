...
router.get('/summary', async (_req, res, next) => {
    try {
        const [v, c, d, o, t, rev, fees, gst, active, available] = await Promise.all([
            query('SELECT COUNT(*)::text count FROM vendors WHERE status=\'ACTIVE\''),
            query(`SELECT COUNT(*)::text count FROM users WHERE role='CUSTOMER' AND is_active=true`),
            query(`SELECT COUNT(*)::text count FROM users WHERE role='DELIVERY_PARTNER' AND is_active=true`),
            query('SELECT COUNT(*)::text count FROM orders'),
            query(`SELECT COUNT(*)::text count FROM support_tickets WHERE status NOT IN ('RESOLVED','CLOSED')`),
            query(`SELECT COALESCE(SUM(total_paise),0)::text sum FROM orders WHERE status NOT IN ('CANCELLED','REFUNDED')`),
            query(`SELECT COALESCE(SUM(amount_paise),0)::text sum FROM ledger_entries WHERE entry_type='PLATFORM_FEE' AND direction='CREDIT'`),
            query(`SELECT COALESCE(SUM(amount_paise),0)::text sum FROM ledger_entries WHERE entry_type='PLATFORM_GST' AND direction='CREDIT'`),
...
router.patch('/orders/:id/status', requirePermission('MANAGE_ORDERS'), async (req, res, next) => {
    try {
        const id = z.string().uuid().parse(req.params.id);
        ...
        const current = await query('SELECT status::text FROM orders WHERE id=$1 FOR UPDATE', [id]);
        if (!current.rows[0]) {
            res.status(404).json({ error: 'ORDER_NOT_FOUND' });
            return;
        }
        ...
        try {
            ...
            if (input.status === 'CANCELLED')
                await releaseInventoryForOrder(client, id);
            if (input.status === 'DELIVERED')
                await commitInventoryForOrder(client, id);
...
router.get('/customers/:customerId', requirePermission('VIEW_CUSTOMER_PROFILE'), async (req, res, next) => {
    try {
        const customerId = z.string().min(5).max(64).parse(req.params.customerId);
        const r = await query(`SELECT user_id FROM customer_profiles WHERE customer_code=$1 LIMIT 1`, [customerId]);
        if (!r.rows[0]) {
...
router.patch('/vendors/:id/status', requirePermission('MANAGE_VENDORS'), async (req, res, next) => {
    try {
        const id = z.string().uuid().parse(req.params.id);
        const input = z.object({ status: z.enum(['PENDING', 'ACTIVE', 'SUSPENDED', 'REJECTED']), reason: z.string().trim().max(500).optional() }).parse(req.body);
        const r = await query(`UPDATE vendors SET status=$1::vendor_status,updated_at=NOW() WHERE id=$2 RETURNING owner_user_id,status::text`, [input.status, id]);
        if (!r.rows[0]) {
        ...
        if (r.rows[0].owner_user_id)
            await queueNotification({ userId: r.rows[0].owner_user_id, channel: 'PUSH', title: 'AasPass vendor status updated', body: `Your store is now ${input.status}.`, data: { type: 'VENDOR_STATUS', vendorId: id, status: input.status } });
...
router.patch('/vendors/:id/kyc', requirePermission('MANAGE_VENDORS'), async (req, res, next) => {
    try {
        const id = z.string().uuid().parse(req.params.id);
        const input = z.object({ status: z.enum(['PENDING', 'VERIFIED', 'REJECTED']), notes: z.string().max(2000).optional() }).parse(req.body);
...
router.patch('/customers/:customerId/status', requirePermission('MANAGE_USERS'), async (req, res, next) => {
    try {
        const customerId = z.string().min(5).max(64).parse(req.params.customerId);
        const input = z.object({ isActive: z.boolean(), reason: z.string().trim().max(500).optional() }).parse(req.body);
        const r = await query(`UPDATE users SET is_active=$1,updated_at=NOW() WHERE id=(SELECT user_id FROM customer_profiles WHERE customer_code=$2 LIMIT 1) AND role='CUSTOMER' RETURNING id AS user_id`, [input.isActive, customerId]);
        if (!r.rows[0]) {
            res.status(404).json({ error: 'CUSTOMER_NOT_FOUND' });
            return;
        }
...
router.patch('/delivery/:id/status', requirePermission('MANAGE_DELIVERY'), async (req, res, next) => {
    try {
        const id = z.string().uuid().parse(req.params.id);
        const input = z.object({ status: z.enum(['OFFLINE', 'AVAILABLE', 'BUSY']), reason: z.string().trim().max(500).optional() }).parse(req.body);
        const r = await query(`UPDATE delivery_partners SET status=$1,last_seen_at=NOW(),updated_at=NOW() WHERE id=$2 RETURNING user_id,status`, [input.status, id]);
        if (!r.rows[0]) {
...
router.get('/finance', requirePermission('VIEW_SETTLEMENTS'), async (_req, res, next) => {
    try {
...
router.get('/people', requirePermission('MANAGE_USERS'), async (_req, res, next) => {
    try {
...
router.get('/audit', requirePermission('VIEW_DASHBOARD'), async (req, res, next) => {
    try {
...
router.post('/customers/:customerId/notify', requirePermission('MANAGE_USERS'), async (req, res, next) => {
    try {
        const customerId = z.string().min(5).max(64).parse(req.params.customerId);
        ...
        const match = await query(`SELECT user_id FROM customer_profiles WHERE customer_code=$1 LIMIT 1`, [customerId]);
        if (!match.rows[0]) {
            res.status(404).json({ error: 'CUSTOMER_NOT_FOUND' });
            return;
        }
        ...
        if (!id) {
            res.status(409).json({ error: 'NOTIFICATION_BLOCKED', message: 'Customer notification preferences do not allow this channel' });
            return;
        }
...
router.get('/gift-cards', requirePermission('MANAGE_GIFT_CARDS'), async (_req, res, next) => {
    try {
...
router.post('/gift-cards', requirePermission('MANAGE_GIFT_CARDS'), async (req, res, next) => {
    try {
        ...
        if (input.customerCode) {
            const c = await query(`SELECT user_id FROM customer_profiles WHERE customer_code=$1 LIMIT 1`, [input.customerCode]);
            if (!c.rows[0]) {
                res.status(404).json({ error: 'CUSTOMER_NOT_FOUND' });
                return;
            }
    ...
    VALUES($1,$2,$3,$3,$4,$5,$6) RETURNING id,code_last4,initial_balance_paise,balance_paise,currency,status,issued_to_user_id,expires_at,created_at`, [hash, raw.slice(-4), input.amountPaise, issuedToUserId, input.expiresAt ?? null, req.authUser.id]);
        const card = r.rows[0];
        ...
        if (issuedToUserId)
            await queueNotification({ userId: issuedToUserId, channel: 'PUSH', title: 'AasPass Gift Card received', body: `A gift card worth ₹${(input.amountPaise / 100).toFixed(2)} has been added to your account.`, data: { type: 'GIFT_CARD_ISSUED', giftCardId: card.id } });
...
router.patch('/people/:id/role', requirePermission('MANAGE_USERS'), async (req, res, next) => {
    try {
        const id = z.string().uuid().parse(req.params.id);
        ...
        if (id === req.authUser.id && input.role !== 'SUPER_ADMIN') {
            res.status(400).json({ error: 'CANNOT_DOWNGRADE_SELF' });
            return;
        }
        const current = await query(`SELECT role::text FROM users WHERE id=$1 LIMIT 1`, [id]);
        if (!current.rows[0]) {
        ...
        const r = await query(`UPDATE users SET role=$1::user_role,updated_at=NOW() WHERE id=$2 RETURNING id,role::text,is_active`, [input.role, id]);
...
router.patch('/people/:id/status', requirePermission('MANAGE_USERS'), async (req, res, next) => {
    try {
        const id = z.string().uuid().parse(req.params.id);
        const input = z.object({ isActive: z.boolean(), reason: z.string().trim().max(500).optional() }).parse(req.body);
        if (id === req.authUser.id && !input.isActive) {
            res.status(400).json({ error: 'CANNOT_DISABLE_SELF' });
            return;
        }
        const r = await query(`UPDATE users SET is_active=$1,updated_at=NOW() WHERE id=$2 RETURNING id,is_active,role::text`, [input.isActive, id]);
        if (!r.rows[0]) {
            res.status(404).json({ error: 'USER_NOT_FOUND' });
            return;
        }
