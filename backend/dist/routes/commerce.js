...
router.post('/promotions/validate', async (req, res, next) => {
    try {
        ...
        const r = await query(`SELECT p.*, (SELECT COUNT(*) FROM promotion_redemptions pr WHERE pr.promotion_id=p.id) total_redemptions, (SELECT COUNT(*) FROM promotion_redemptions pr WHERE pr.promotion_id=p.id AND pr.user_id=$1) customer_redemptions FROM promotions p WHERE p.code=$2 LIMIT 1`, [req.authUser.id, input.code]);
        const p = r.rows[0];
        if (!p) {
            res.status(404).json({ error: 'PROMOTION_NOT_FOUND' });
            return;
        }
        if (!p.is_active || new Date(p.starts_at) > new Date() || (p.ends_at && new Date(p.ends_at) <= new Date())) {
            res.status(409).json({ error: 'PROMOTION_INACTIVE' });
            return;
        }
        if (p.usage_limit != null && Number(p.total_redemptions) >= Number(p.usage_limit)) {
            res.status(409).json({ error: 'PROMOTION_EXHAUSTED' });
            return;
        }
        if (Number(p.customer_redemptions) >= Number(p.per_customer_limit)) {
            res.status(409).json({ error: 'PROMOTION_CUSTOMER_LIMIT' });
            return;
        }
        if (input.orderSubtotalPaise < Number(p.min_order_paise)) {
            res.status(409).json({ error: 'PROMOTION_MIN_ORDER', minimumPaise: Number(p.min_order_paise) });
            return;
        }
...
router.post('/wallet/admin-credit', async (req, res, next) => {
    try {
        if (!['SUPER_ADMIN', 'ADMIN'].includes(req.authUser.role)) {
        ...
        try {
            ...
            const user = await client.query(`SELECT user_id id FROM customer_profiles WHERE customer_code=$1 LIMIT 1`, [input.customerId]);
            if (!user.rows[0]) {
                await client.query('ROLLBACK');
                res.status(404).json({ error: 'CUSTOMER_NOT_FOUND' });
                return;
            }
            const wallet = await client.query(`INSERT INTO wallet_accounts(user_id) VALUES($1) ON CONFLICT(user_id) DO UPDATE SET updated_at=NOW() RETURNING id`, [user.rows[0].id]);
            const existing = await client.query(`SELECT id FROM wallet_transactions WHERE idempotency_key=$1`, [input.idempotencyKey]);
            if (existing.rows[0]) {
                await client.query('COMMIT');
                res.json({ ok: true, reused: true, transactionId: existing.rows[0].id });
                return;
            }
            const tx = await client.query(`INSERT INTO wallet_transactions(wallet_id,direction,amount_paise,transaction_type,reference_type,reference_id,idempotency_key,metadata) VALUES($1,'CREDIT',$2,'ADMIN_CREDIT','CUSTOMER',$3,$4,$5) RETURNING id`, [wallet.rows[0].id, input.amountPaise, input.customerId, input.idempotencyKey, JSON.stringify({ reason: input.reason, actor: req.authUser.id })]);
            await client.query(`UPDATE wallet_accounts SET balance_paise=balance_paise+$1,updated_at=NOW() WHERE id=$2`, [input.amountPaise, wallet.rows[0].id]);
            await client.query('COMMIT');
            res.status(201).json({ ok: true, transactionId: tx.rows[0].id });
        }
        catch (e) {
            await client.query('ROLLBACK');
            throw e;
