...
const ticketStatuses = ['OPEN', 'IN_PROGRESS', 'WAITING_CUSTOMER', 'WAITING_VENDOR', 'WAITING_DELIVERY', 'RESOLVED', 'CLOSED'];
...
function canAccessAllTickets(role) {
...
function canLookupCustomer(role) {
    return ['SUPER_ADMIN', 'ADMIN', 'SUPPORT_LEAD', 'SUPPORT_AGENT'].includes(role);
}
router.get('/customers/lookup', requirePermission(PERMISSIONS.VIEW_CUSTOMER_PROFILE), async (req, res, next) => {
    try {
        if (!canLookupCustomer(req.authUser.role)) {
...
router.get('/customers/:customerId', requirePermission(PERMISSIONS.VIEW_CUSTOMER_PROFILE), async (req, res, next) => {
    try {
        if (!canLookupCustomer(req.authUser.role)) {
            res.status(403).json({ error: 'FORBIDDEN' });
            return;
        ...
        const phone = await query(`SELECT phone FROM users WHERE id=$1 LIMIT 1`, [match.rows[0].user_id]);
...
router.get('/agents', requireRoles('SUPER_ADMIN', 'ADMIN', 'SUPPORT_LEAD'), async (_req, res, next) => {
    try {
...
router.post('/:id/messages', async (req, res, next) => {
    try {
        ...
        const result = await query(`INSERT INTO support_ticket_messages(ticket_id,sender_user_id,message,is_internal) VALUES($1,$2,$3,FALSE) RETURNING id`, [ticketId, req.authUser.id, input.message]);
...
router.patch('/:id', requireRoles('SUPER_ADMIN', 'ADMIN', 'SUPPORT_LEAD', 'SUPPORT_AGENT'), async (req, res, next) => {
    try {
        if (req.authUser.role === 'SUPPORT_AGENT' && req.body.assignedTo) {
            res.status(403).json({ error: 'FORBIDDEN', message: 'Support agents cannot reassign tickets' });
            return;
        }
        const ticketId = z.string().uuid().parse(req.params.id);
        ...
        if (input.priority) {
            setParts.push(`priority=$${i++}`);
            params.push(input.priority);
        }
        if (input.assignedTo !== undefined) {
            setParts.push(`assigned_to=$${i++}`);
        ...
        await audit('SUPPORT_TICKET_UPDATED', 'support_tickets', ticketId, req.authUser.id, input);
        const requester = await query(`SELECT requester_user_id FROM support_tickets WHERE id=$1`, [ticketId]);
...
router.post('/:id/internal-note', requireRoles('SUPER_ADMIN', 'ADMIN', 'SUPPORT_LEAD', 'SUPPORT_AGENT'), async (req, res, next) => {
    try {
        const ticketId = z.string().uuid().parse(req.params.id);
        const input = messageSchema.parse(req.body);
        const result = await query(`INSERT INTO support_ticket_messages(ticket_id,sender_user_id,message,is_internal) VALUES($1,$2,$3,TRUE) RETURNING id`, [ticketId, req.authUser.id, input.message]);
