...
router.post('/payments/session', async (req, res, next) => {
    try {
        ...
        if (!cashfreeConfigured()) {
            res.status(503).json({ error: 'PAYMENT_PROVIDER_NOT_CONFIGURED' });
            return;
        }
        const result = await query('SELECT o.id,o.customer_id,o.total_paise,u.phone,u.name,u.email FROM orders o JOIN users u ON u.id=o.customer_id WHERE o.id=$1 LIMIT 1', [input.orderId]);
        const order = result.rows[0];
        if (!order) {
            res.status(404).json({ error: 'ORDER_NOT_FOUND' });
            return;
        }
        if (req.authUser.id !== order.customer_id && !['SUPER_ADMIN', 'ADMIN', 'FINANCE_ADMIN'].includes(req.authUser.role)) {
        ...
        const updatedPayment = await query(`UPDATE payments SET method='ONLINE',status='PENDING',provider='CASHFREE',provider_order_id=$1,amount_paise=$2,currency='INR',raw_response=$3,updated_at=NOW() WHERE order_id=$4 RETURNING id`, [cf.orderId, Number(order.total_paise), JSON.stringify(cf.raw), order.id]);
        if (updatedPayment.rows.length === 0) {
            await query(`INSERT INTO payments(order_id,method,status,provider,provider_order_id,amount_paise,currency,raw_response,updated_at) VALUES($1,'ONLINE','PENDING','CASHFREE',$2,$3,'INR',$4,NOW())`, [order.id, cf.orderId, Number(order.total_paise), JSON.stringify(cf.raw)]);
        }
...
router.get('/payments/:orderId/status', async (req, res, next) => {
    try {
        ...
        const result = await query('SELECT customer_id FROM orders WHERE id=$1 LIMIT 1', [orderId]);
        if (!result.rows[0]) {
            res.status(404).json({ error: 'ORDER_NOT_FOUND' });
            return;
        }
        if (req.authUser.id !== result.rows[0].customer_id && !['SUPER_ADMIN', 'ADMIN', 'FINANCE_ADMIN'].includes(req.authUser.role)) {
            res.status(403).json({ error: 'FORBIDDEN' });
            return;
        }
...
webhookRouter.post('/memberships/webhook', async (req, res, next) => {
    try {
        ...
        const providerSubscriptionId = String(event.subscription_id ?? (event.data?.subscription_id ?? ''));
        const rawStatus = String(event.status ?? event.subscription_status ?? event.data?.status ?? '').toUpperCase();
        const statusMap = { ACTIVE: 'ACTIVE', ACTIVATED: 'ACTIVE', PAID: 'ACTIVE', PAST_DUE: 'PAST_DUE', PAYMENT_FAILED: 'PAST_DUE', CANCELLED: 'CANCELLED', CANCELED: 'CANCELLED', EXPIRED: 'EXPIRED' };
        const status = statusMap[rawStatus];
        if (!providerSubscriptionId || !status) {
            res.status(400).json({ error: 'INVALID_MEMBERSHIP_EVENT' });
            return;
        }
        const existing = await query(`SELECT id,status,user_id FROM customer_memberships WHERE provider_subscription_id=$1 LIMIT 1`, [providerSubscriptionId]);
        if (!existing.rows[0]) {
            res.status(404).json({ error: 'MEMBERSHIP_NOT_FOUND' });
            return;
        }
        const membership = existing.rows[0];
        const eventInsert = await query(`INSERT INTO membership_events(membership_id,event_type,provider_event_id,metadata) VALUES($1,$2,$3,$4) ON CONFLICT(provider_event_id) DO NOTHING RETURNING id`, [membership.id, `PROVIDER_${status}`, providerEventId || null, JSON.stringify(event)]);
        if (providerEventId && !eventInsert.rows[0]) {
        ...
        if (active) {
            await query(`UPDATE customer_memberships SET status='EXPIRED',updated_at=NOW() WHERE user_id=$1 AND id<>$2 AND status='ACTIVE'`, [membership.user_id, membership.id]);
            const { publishDomainEvent } = await import('../../services/integrations/event-bus.js');
            await publishDomainEvent({ type: 'MEMBERSHIP_ACTIVATED', aggregateType: 'CUSTOMER_MEMBERSHIP', aggregateId: membership.id, payload: { providerSubscriptionId, status } });
        }
...
webhookRouter.post('/payments/webhook', async (req, res, next) => {
    try {
        ...
        if (!parsed) {
            res.status(400).json({ error: 'PAYMENT_EVENT_ORDER_ID_MISSING' });
            return;
        }
        const eventKey = req.header('x-webhook-event-id') ?? `${timestamp}:${signature}`;
        const insert = await query(`INSERT INTO payment_webhook_events(provider,event_key,order_id,event_type,payload,received_at) VALUES('CASHFREE',$1,$2,$3,$4,NOW()) ON CONFLICT(provider,event_key) DO UPDATE SET event_key=EXCLUDED.event_key RETURNING id,processed_at`, [eventKey, parsed.orderId, String(event.type ?? event.event_type ?? 'UNKNOWN'), JSON.stringify(event)]);
        const webhookId = insert.rows[0].id;
        if (insert.rows[0].processed_at) {
            res.json({ received: true, duplicate: true, processed: true });
            return;
        }
        const order = await query('SELECT id,total_paise,status::text FROM orders WHERE id=$1 LIMIT 1', [parsed.orderId]);
        if (!order.rows[0]) {
            await query(`UPDATE payment_webhook_events SET processed_at=NOW(),processing_error='ORDER_NOT_FOUND' WHERE id=$1`, [webhookId]);
            res.status(404).json({ error: 'ORDER_NOT_FOUND' });
            return;
        }
        if (parsed.amountPaise != null && parsed.amountPaise !== Number(order.rows[0].total_paise)) {
            await query(`UPDATE payment_webhook_events SET processed_at=NOW(),processing_error='PAYMENT_AMOUNT_MISMATCH' WHERE id=$1`, [webhookId]);
            res.status(409).json({ error: 'PAYMENT_AMOUNT_MISMATCH' });
            return;
        }
        ...
        if (parsed.status === 'PAID') {
            ...
            try {
                ...
                const failedOrder = await client.query(`SELECT customer_id,wallet_applied_paise FROM orders WHERE id=$1 FOR UPDATE`, [parsed.orderId]);
                if (failedOrder.rows[0]) {
                    ...
                    if (walletAmount > 0)
                        await creditWalletForRefund(client, failedOrder.rows[0].customer_id, walletAmount, parsed.orderId);
...
router.post('/notifications/dispatch', requirePermission('MANAGE_SYSTEM_CONFIG'), async (_req, res, next) => {
    try {
        res.json(await dispatchPendingNotifications());
    }
    catch (e) {
        next(e);
    }
...
router.get('/admin/outbox', requirePermission('MANAGE_SYSTEM_CONFIG'), async (req, res, next) => { try {
    const limit = Math.min(200, Math.max(1, Number(req.query.limit ?? 100)));
    const r = await query(`SELECT id,event_type,aggregate_type,aggregate_id,status,attempts,available_at,processed_at,last_error,created_at FROM integration_outbox ORDER BY created_at DESC LIMIT $1`, [limit]);
    res.json({ events: r.rows, mode: 'database' });
}
catch (e) {
    next(e);
} });
router.post('/admin/outbox/dispatch', requirePermission('MANAGE_SYSTEM_CONFIG'), async (_req, res, next) => { try {
    const { dispatchIntegrationOutbox } = await import('../../services/integrations/event-bus.js');
    res.json(await dispatchIntegrationOutbox());
}
catch (e) {
    next(e);
} });
