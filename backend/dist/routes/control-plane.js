...
router.get('/vendor-onboarding', requirePermission('VIEW_VENDORS'), async (_req, res, next) => {
    try {
...
router.patch('/vendors/:id/activation', requirePermission('MANAGE_VENDORS'), async (req, res, next) => {
    try {
        ...
        const current = await query(`SELECT * FROM vendor_activation_checks WHERE vendor_id=$1`, [id]);
...
router.post('/vendors/:id/service-zones', requirePermission('MANAGE_VENDORS'), async (req, res, next) => {
    try {
        const vendorId = z.string().uuid().parse(req.params.id);
        const input = z.object({ serviceZoneId: z.string().uuid(), isActive: z.boolean().default(true) }).parse(req.body);
...
router.get('/delivery-onboarding', requirePermission('MANAGE_DELIVERY'), async (_req, res, next) => {
    try {
...
router.patch('/delivery/:id/kyc', requirePermission('MANAGE_DELIVERY'), async (req, res, next) => {
    try {
        const id = z.string().uuid().parse(req.params.id);
        const input = z.object({ status: z.enum(['PENDING', 'SUBMITTED', 'VERIFIED', 'REJECTED']), notes: z.string().trim().max(1000).optional() }).parse(req.body);
...
router.get('/service-zones', async (_req, res, next) => {
    try {
...
router.post('/service-zones', requirePermission('MANAGE_SYSTEM_CONFIG'), async (req, res, next) => {
    try {
        ...
        await audit('SERVICE_ZONE_CREATED', 'service_zones', r.rows[0].id, req.authUser.id, input);
        res.status(201).json({ zone: r.rows[0] });
    }
    catch (e) {
        next(e);
    }
});
router.get('/inventory', requirePermission('VIEW_VENDORS'), async (req, res, next) => {
    try {
...
router.post('/promotions', requirePermission('MANAGE_SYSTEM_CONFIG'), async (req, res, next) => {
    try {
        ...
        await audit('PROMOTION_CREATED', 'promotions', r.rows[0].id, req.authUser.id, { code: input.code });
        res.status(201).json({ promotion: r.rows[0] });
    }
    catch (e) {
        next(e);
    }
});
router.patch('/promotions/:id', requirePermission('MANAGE_SYSTEM_CONFIG'), async (req, res, next) => {
    try {
        const id = z.string().uuid().parse(req.params.id);
        const input = z.object({ isActive: z.boolean(), endsAt: z.string().datetime().optional().nullable() }).parse(req.body);
        ...
        await audit('PROMOTION_UPDATED', 'promotions', id, req.authUser.id, input);
        res.json({ promotion: r.rows[0] });
    }
    catch (e) {
        next(e);
    }
});
router.get('/settlements', requirePermission('VIEW_SETTLEMENTS'), async (_req, res, next) => {
    try {
...
router.post('/settlements/generate', requirePermission('MANAGE_SETTLEMENTS'), async (req, res, next) => {
    try {
        ...
        try {
            ...
            for (const x of partners.rows) {
                const net = Number(x.base) + Number(x.bonus);
                await client.query(`INSERT INTO delivery_settlements(delivery_partner_id,period_start,period_end,base_earning_paise,bonus_paise,net_payout_paise) VALUES($1,$2,$3,$4,$5,$6) ON CONFLICT(delivery_partner_id,period_start,period_end) DO UPDATE SET base_earning_paise=EXCLUDED.base_earning_paise,bonus_paise=EXCLUDED.bonus_paise,net_payout_paise=EXCLUDED.net_payout_paise`, [x.id, input.periodStart, input.periodEnd, x.base, x.bonus, net]);
            }
        ...
        await audit('SETTLEMENTS_GENERATED', 'settlements', null, req.authUser.id, input);
        res.json({ ok: true, periodStart: input.periodStart, periodEnd: input.periodEnd });
    }
    catch (e) {
        next(e);
    }
...
router.get('/risk', async (_req, res, next) => {
    try {
        ...
        res.json({ flags: r.rows, mode: 'database' });
    }
    catch (e) {
        next(e);
    }
});
router.post('/risk/scan', requirePermission('MANAGE_SYSTEM_CONFIG'), async (req, res, next) => {
    try {
        const customers = await query(`SELECT u.id user_id,COUNT(o.id)::int order_count,COUNT(o.id) FILTER(WHERE o.status='CANCELLED')::int cancel_count,COUNT(o.id) FILTER(WHERE o.status='REFUNDED')::int refund_count,cp.customer_code FROM users u JOIN customer_profiles cp ON cp.user_id=u.id LEFT JOIN orders o ON o.customer_id=u.id AND o.created_at>=NOW()-INTERVAL '30 days' WHERE u.role='CUSTOMER' GROUP BY u.id,cp.customer_code HAVING COUNT(o.id)>=5`);
        let created = 0;
        for (const c of customers.rows) {
            const score = Math.min(100, c.cancel_count * 12 + c.refund_count * 15);
            if (score < 60)
                continue;
            const severity = score >= 85 ? 'CRITICAL' : score >= 70 ? 'HIGH' : 'MEDIUM';
            const existing = await query(`SELECT id FROM risk_flags WHERE user_id=$1 AND risk_type='ORDER_BEHAVIOR' AND status='OPEN' LIMIT 1`, [c.user_id]);
            if (existing.rows[0])
                continue;
            await query(`INSERT INTO risk_flags(user_id,risk_type,score,severity,reason) VALUES($1,'ORDER_BEHAVIOR',$2,$3,$4)`, [c.user_id, score, severity, `30-day pattern: ${c.cancel_count} cancellations and ${c.refund_count} refunds across ${c.order_count} orders.`]);
            created++;
        }
        await audit('RISK_SCAN_RUN', 'risk_flags', null, req.authUser.id, { created });
        res.json({ created });
    }
    catch (e) {
        next(e);
...
router.patch('/risk/:id', requirePermission('MANAGE_SYSTEM_CONFIG'), async (req, res, next) => {
    try {
        const id = z.string().uuid().parse(req.params.id);
        const input = z.object({ status: z.enum(['REVIEWED', 'RESOLVED', 'DISMISSED']), note: z.string().trim().max(1000).optional() }).parse(req.body);
        const r = await query(`UPDATE risk_flags SET status=$1,reviewed_by=$2,reviewed_at=NOW(),reason=CASE WHEN $3::text IS NULL OR $3='' THEN reason ELSE reason||' | Review: '||$3 END WHERE id=$4 RETURNING *`, [input.status, req.authUser.id, input.note ?? null, id]);
        if (!r.rows[0]) {
            res.status(404).json({ error: 'RISK_FLAG_NOT_FOUND' });
            return;
        }
        await audit('RISK_FLAG_UPDATED', 'risk_flags', id, req.authUser.id, input);
