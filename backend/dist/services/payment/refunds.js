...
export async function createProviderRefund(input) {
    ...
    if (!response.ok)
        throw new Error(`CASHFREE_REFUND_FAILED:${response.status}:${JSON.stringify(raw)}`);
    return raw;
}
export async function requestRefund(input) {
    const order = await query(`SELECT total_paise,status::text,customer_id,wallet_applied_paise,wallet_refund_paise,cashfree_refund_paise FROM orders WHERE id=$1 LIMIT 1`, [input.orderId]);
    const row = order.rows[0];
    if (!row)
        throw new Error('ORDER_NOT_FOUND');
    if (!['PAID', 'DELIVERED', 'CANCELLED', 'REFUND_PENDING', 'REFUNDED'].includes(row.status))
        throw new Error('ORDER_NOT_REFUNDABLE');
    if (input.amountPaise <= 0 || input.amountPaise > Number(row.total_paise))
    ...
    if (providerRefund > 0) {
        ...
        if (providerRefund > providerCap)
            throw new Error('INVALID_PROVIDER_REFUND_AMOUNT');
    }
    const existing = await query(`SELECT id,status,provider_ref FROM refunds WHERE order_id=$1 AND status IN ('PENDING','PROCESSING') LIMIT 1`, [input.orderId]);
    if (existing.rows[0])
        return { ...existing.rows[0], walletRefundPaise: 0, providerRefundPaise: Number(row.cashfree_refund_paise || 0) };
    const client = await pool.connect();
    let refundId = null;
    try {
        await client.query('BEGIN');
        if (walletRefund > 0)
            await creditWalletForRefund(client, row.customer_id, walletRefund, input.orderId);
    ...
    try {
        ...
        if (internal === 'PROCESSED')
            await finalizeRefund(input.orderId, refundId);
...
export async function finalizeRefund(orderId, refundId) {
...
export async function syncProviderRefund(refundId) {
    const row = await query(`SELECT id,order_id,provider_ref,status FROM refunds WHERE id=$1 LIMIT 1`, [refundId]);
    const refund = row.rows[0];
    if (!refund)
        throw new Error('REFUND_NOT_FOUND');
    if (!refund.provider_ref)
        throw new Error('REFUND_PROVIDER_REFERENCE_MISSING');
    if (!configured())
        throw new Error('CASHFREE_NOT_CONFIGURED');
    ...
    const raw = await response.json();
    if (!response.ok)
        throw new Error(`CASHFREE_REFUND_STATUS_FAILED:${response.status}:${JSON.stringify(raw)}`);
    const providerStatus = String(raw.refund_status ?? raw.status ?? 'PENDING').toUpperCase();
    ...
    if (internal === 'PROCESSED')
        await finalizeRefund(refund.order_id, refund.id);
