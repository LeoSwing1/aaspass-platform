...
export async function assignNearestDeliveryPartner(orderId) {
    if (!pool)
    ...
    `, [orderId]);
        ...
        const existing = await client.query(`SELECT id,delivery_partner_id FROM delivery_assignments WHERE order_id=$1 FOR UPDATE`, [orderId]);
    ...
    `);
        ...
        const assignment = existingAssignment
            ? await client.query(`UPDATE delivery_assignments SET delivery_partner_id=$1,status='OFFERED',offer_expires_at=NOW()+INTERVAL '90 seconds',updated_at=NOW() WHERE id=$2 RETURNING id`, [best.id, existingAssignment.id])
            : await client.query(`INSERT INTO delivery_assignments(order_id,delivery_partner_id,status,pickup_latitude,pickup_longitude,drop_latitude,drop_longitude,earning_paise,offer_expires_at) VALUES($1,$2,'OFFERED',$3,$4,$5,$6,$7,NOW()+INTERVAL '90 seconds') RETURNING id`, [orderId, best.id, row.vendor_latitude, row.vendor_longitude, row.drop_latitude, row.drop_longitude, Math.max(500, Math.floor(Number(row.delivery_fee_paise) * 0.7))]);
