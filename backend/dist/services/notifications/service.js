...
async function getFcmAccessToken() {
    ...
    if (!response.ok || !raw.access_token)
        throw new Error(`FCM_AUTH_FAILED:${response.status}:${raw.error ?? 'unknown'}`);
    cachedFcmToken = { value: raw.access_token, expiresAt: Date.now() + Math.max(60, raw.expires_in ?? 3600) * 1000 };
    return raw.access_token;
}
export function fcmConfigured() { return Boolean(env.FCM_PROJECT_ID && env.FCM_CLIENT_EMAIL && env.FCM_PRIVATE_KEY); }
export function whatsappConfigured() { return Boolean(env.WHATSAPP_PHONE_NUMBER_ID && env.WHATSAPP_ACCESS_TOKEN); }
export async function sendFcm(token, title, body, data = {}) {
    ...
    if (!response.ok || !raw.name)
        throw new Error(`FCM_SEND_FAILED:${response.status}:${raw.error?.message ?? 'unknown'}`);
...
export async function sendWhatsAppText(phone, body) {
    ...
    if (!response.ok || !raw.messages?.[0]?.id)
        throw new Error(`WHATSAPP_SEND_FAILED:${response.status}:${raw.error?.message ?? 'unknown'}`);
    return { messageId: raw.messages[0].id };
}
async function preferenceAllows(userId, channel) {
    const result = await query('SELECT push_enabled,whatsapp_enabled,sms_enabled,email_enabled FROM customer_notification_preferences WHERE user_id=$1', [userId]);
...
export async function queueNotification(input) {
    if (!(await preferenceAllows(input.userId, input.channel)))
        return null;
    const result = await query(`INSERT INTO notifications(user_id,channel,title,body,data,status) VALUES($1,$2,$3,$4,$5,'PENDING') RETURNING id`, [input.userId, input.channel, input.title, input.body, JSON.stringify(input.data ?? {})]);
    const id = result.rows[0]?.id;
    if (id && input.dispatchNow !== false && env.NOTIFICATION_DISPATCH_ENABLED)
        void dispatchNotification(id);
    return id ?? null;
}
export async function dispatchNotification(notificationId) {
    const result = await query(`SELECT n.*,u.phone FROM notifications n JOIN users u ON u.id=n.user_id WHERE n.id=$1 LIMIT 1`, [notificationId]);
    const item = result.rows[0];
    if (!item)
        return;
    const attempt = await query(`SELECT COALESCE(MAX(attempt_number),0)::int+1 AS n FROM notification_delivery_attempts WHERE notification_id=$1`, [notificationId]);
    const attemptNumber = attempt.rows[0]?.n ?? 1;
    try {
        if (item.channel === 'PUSH') {
            const tokens = await query(`SELECT id,token FROM device_tokens WHERE user_id=$1 AND is_active=true`, [item.user_id]);
            if (!fcmConfigured()) {
                await query(`UPDATE notifications SET status='QUEUED' WHERE id=$1`, [notificationId]);
                await query(`INSERT INTO notification_delivery_attempts(notification_id,provider,status,attempt_number,error_message) VALUES($1,'FCM','QUEUED',$2,'FCM credentials not configured')`, [notificationId, attemptNumber]);
                return;
            }
            if (tokens.rows.length === 0) {
                await query(`UPDATE notifications SET status='SKIPPED' WHERE id=$1`, [notificationId]);
                await query(`INSERT INTO notification_delivery_attempts(notification_id,provider,status,attempt_number,error_message) VALUES($1,'FCM','SKIPPED',$2,'No active device token')`, [notificationId, attemptNumber]);
                return;
            }
            let delivered = 0;
            for (const token of tokens.rows) {
                try {
                    ...
                    await query(`INSERT INTO notification_delivery_attempts(notification_id,provider,status,provider_message_id,attempt_number) VALUES($1,'FCM','SENT',$2,$3)`, [notificationId, sent.messageId, attemptNumber]);
                    delivered++;
                }
                catch (error) {
                    ...
                    if (/registration token is not registered|unregistered|not found/i.test(message))
                        await query(`UPDATE device_tokens SET is_active=false,updated_at=NOW() WHERE id=$1`, [token.id]);
                }
            }
...
export async function dispatchPendingNotifications(limit = env.NOTIFICATION_DISPATCH_BATCH_SIZE) {
    const client = await pool.connect();
    let ids = [];
    try {
        await client.query('BEGIN');
        const rows = await client.query(`SELECT id FROM notifications WHERE status='PENDING' AND created_at<=NOW() ORDER BY created_at ASC FOR UPDATE SKIP LOCKED LIMIT $1`, [limit]);
        ids = rows.rows.map(r => r.id);
        if (ids.length)
            await client.query(`UPDATE notifications SET status='PROCESSING' WHERE id = ANY($1::uuid[])`, [ids]);
