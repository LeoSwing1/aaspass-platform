...
export async function requireAuth(req, res, next) {
    ...
    if (!header?.startsWith('Bearer ')) {
        res.status(401).json({ error: 'AUTH_REQUIRED', message: 'Bearer token required', requestId: req.requestId });
        return;
    }
    const payload = verifyAccessToken(header.slice(7).trim(), env.JWT_SECRET, env.JWT_ISSUER, env.JWT_AUDIENCE);
    if (!payload) {
        res.status(401).json({ error: 'INVALID_TOKEN', message: 'Session expired or invalid', requestId: req.requestId });
        return;
    }
    if (env.DATABASE_URL) {
        const session = await query('SELECT revoked_at, expires_at FROM auth_sessions WHERE jti=$1 LIMIT 1', [payload.jti]);
...
export function requireRoles(...roles) {
    return (req, res, next) => {
        ...
        if (!roles.includes(req.authUser.role)) {
            res.status(403).json({ error: 'FORBIDDEN', message: 'Insufficient permissions', requestId: req.requestId });
            return;
        }
        next();
    };
