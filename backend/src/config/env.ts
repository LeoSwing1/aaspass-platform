...
try { loadEnvFile('.env'); } catch { /* Host/CI environment variables may be supplied without a file. */ }
...
if (isProduction) {
  if (env.DEV_AUTH_ENABLED) throw new Error('DEV_AUTH_ENABLED must be false in production');
  if (env.JWT_SECRET === 'dev-only-change-this-secret-before-production-please-123456') throw new Error('JWT_SECRET must be rotated before production');
}
