...
const numberFromEnv = <T>(schema: z.ZodType<T>) =>
  z.preprocess(blankAsUndefined, schema);
...
const schema = z.object({
  ...
  PORT: numberFromEnv(
    z.coerce.number().int().positive().default(4100)
  ),
  ...
  DEFAULT_SERVICE_RADIUS_KM: numberFromEnv(
    z.coerce.number().positive().default(10)
  ),
  ...
  NOTIFICATION_DISPATCH_BATCH_SIZE: numberFromEnv(
    z.coerce.number().int().positive().default(25)
  ),
  ...
  DELIVERY_OTP_SECRET: z.string().min(32).default(
