...
router.get('/loyalty', requireRoles('CUSTOMER','SUPER_ADMIN','ADMIN'), async(req,res,next)=>{try{
...
router.post('/loyalty/redeem-to-wallet', requireRoles('CUSTOMER'), async(req,res,next)=>{const client=await pool.connect();try{
  const input=z.object({points:z.number().int().positive().max(100000),idempotencyKey:z.string().min(8).max(120)}).parse(req.body);
  await client.query('BEGIN');
  const account=await client.query<{id:string;points:number}>(`SELECT id,points FROM loyalty_accounts WHERE user_id=$1 FOR UPDATE`,[req.authUser!.id]);
  if(!account.rows[0]){await client.query('ROLLBACK');res.status(404).json({error:'LOYALTY_ACCOUNT_NOT_FOUND'});return;}
  ...
  if(existing.rows[0]){await client.query('COMMIT');res.json({reused:true,transactionId:existing.rows[0].id});return;}
  if(Number(account.rows[0].points)<input.points){await client.query('ROLLBACK');res.status(409).json({error:'INSUFFICIENT_LOYALTY_POINTS',availablePoints:Number(account.rows[0].points)});return;}
  // 100 loyalty points = ₹1 wallet credit. Both ledger entries are atomic.
  const wallet=await client.query<{id:string}>(`INSERT INTO wallet_accounts(user_id) VALUES($1) ON CONFLICT(user_id) DO UPDATE SET updated_at=NOW() RETURNING id`,[req.authUser!.id]);
  const amount=Math.floor(input.points/100)*100;
  if(amount<100){await client.query('ROLLBACK');res.status(400).json({error:'MINIMUM_REDEMPTION_100_POINTS'});return;}
...
router.get('/referral', requireRoles('CUSTOMER','SUPER_ADMIN','ADMIN'), async(req,res,next)=>{try{
  const existing=await query<{id:string;referral_code:string}>(`SELECT id,referral_code FROM referral_accounts WHERE user_id=$1`,[req.authUser!.id]);
  let code=existing.rows[0]?.referral_code;
  if(!code){
    code=referralCode(req.authUser!.id);
    let attempt=0;
    while(attempt<5){try{const r=await query<{referral_code:string}>(`INSERT INTO referral_accounts(user_id,referral_code) VALUES($1,$2) RETURNING referral_code`,[req.authUser!.id,code]);code=r.rows[0].referral_code;break;}catch(e){attempt++;code=`AAS${req.authUser!.id.replace(/-/g,'').slice(0,7).toUpperCase()}${attempt}`;}}
  }
...
router.post('/referral/apply', requireRoles('CUSTOMER'), async(req,res,next)=>{try{
  const input=z.object({code:z.string().trim().toUpperCase().min(4).max(24)}).parse(req.body);
  const ref=await query<{user_id:string}>(`SELECT user_id FROM referral_accounts WHERE referral_code=$1`,[input.code]);
  if(!ref.rows[0]){res.status(404).json({error:'REFERRAL_NOT_FOUND'});return;}
  if(ref.rows[0].user_id===req.authUser!.id){res.status(409).json({error:'SELF_REFERRAL'});return;}
  const existing=await query(`SELECT id FROM referral_events WHERE referred_user_id=$1`,[req.authUser!.id]);
  if(existing.rows[0]){res.status(409).json({error:'REFERRAL_ALREADY_APPLIED'});return;}
...
router.get('/membership-plans', requireRoles('CUSTOMER','SUPER_ADMIN','ADMIN'), async(_req,res,next)=>{try{
...
router.get('/membership', requireRoles('CUSTOMER','SUPER_ADMIN','ADMIN'), async(req,res,next)=>{try{
...
router.post('/membership/start', requireRoles('CUSTOMER'), async(req,res,next)=>{try{
  ...
  if(!plan.rows[0]){res.status(404).json({error:'MEMBERSHIP_PLAN_NOT_FOUND'});return;}
  const active=await query(`SELECT id FROM customer_memberships WHERE user_id=$1 AND status='ACTIVE' LIMIT 1`,[req.authUser!.id]);
  if(active.rows[0]){res.status(409).json({error:'MEMBERSHIP_ALREADY_ACTIVE',membershipId:active.rows[0].id});return;}
  ...
  if(pending.rows[0]){res.status(409).json({error:'MEMBERSHIP_CHECKOUT_ALREADY_PENDING',membershipId:pending.rows[0].id});return;}
...
router.post('/admin/memberships/:id/provider-state', requirePermission('MANAGE_SETTLEMENTS'), async(req,res,next)=>{try{
  ...
  const client=await pool.connect(); try{await client.query('BEGIN');
    if(input.providerEventId){const replay=await client.query(`SELECT id FROM membership_events WHERE provider_event_id=$1`,[input.providerEventId]);if(replay.rows[0]){await client.query('COMMIT');res.json({ok:true,replayed:true});return;}}
    if(input.status==='ACTIVE') await client.query(`UPDATE customer_memberships SET status='CANCELLED',updated_at=NOW() WHERE user_id=(SELECT user_id FROM customer_memberships WHERE id=$1) AND status='ACTIVE' AND id<>$1`,[id]);
    ...
    if(!r.rows[0]){await client.query('ROLLBACK');res.status(404).json({error:'MEMBERSHIP_NOT_FOUND'});return;}
    ...
    await client.query('COMMIT'); await audit('CUSTOMER_MEMBERSHIP_PROVIDER_STATE','customer_memberships',id,req.authUser!.id,input); res.json({membership:r.rows[0]});
  }catch(e){await client.query('ROLLBACK');throw e;}finally{client.release();}
}catch(e){next(e)}});

router.get('/admin/campaigns', requirePermission('MANAGE_SYSTEM_CONFIG'), async(_req,res,next)=>{try{
...
router.post('/admin/campaigns', requirePermission('MANAGE_SYSTEM_CONFIG'), async(req,res,next)=>{try{
  ...
  await audit('CAMPAIGN_CREATED','promotions',r.rows[0].id,req.authUser!.id,{code:input.code,audience:input.audience});res.status(201).json({campaign:r.rows[0]});
}catch(e){next(e)}});

router.patch('/admin/campaigns/:id', requirePermission('MANAGE_SYSTEM_CONFIG'), async(req,res,next)=>{try{
  const id=z.string().uuid().parse(req.params.id); const input=z.object({isActive:z.boolean().optional(),endsAt:z.string().datetime().optional().nullable(),membershipRequired:z.boolean().optional(),minOrders:z.number().int().nonnegative().optional(),minSpendPaise:z.number().int().nonnegative().optional(),loyaltyMinPoints:z.number().int().nonnegative().optional()}).strict().parse(req.body);
  const current=await query(`SELECT * FROM promotions WHERE id=$1`,[id]);if(!current.rows[0]){res.status(404).json({error:'CAMPAIGN_NOT_FOUND'});return;}
  const c={...current.rows[0],...input}; const r=await query(`UPDATE promotions SET is_active=$1,ends_at=$2,membership_required=$3,min_orders=$4,min_spend_paise=$5,loyalty_min_points=$6,updated_at=NOW() WHERE id=$7 RETURNING *`,[c.isActive,c.endsAt??null,c.membershipRequired,c.minOrders,c.minSpendPaise,c.loyaltyMinPoints,id]);
  await audit('CAMPAIGN_UPDATED','promotions',id,req.authUser!.id,input);res.json({campaign:r.rows[0]});
}catch(e){next(e)}});

router.get('/admin/vendor-analytics/:vendorId', requirePermission('VIEW_DASHBOARD'), async(req,res,next)=>{try{
  const vendorId=z.string().uuid().parse(req.params.vendorId);const days=Math.min(90,Math.max(7,Number(req.query.days||30)));
...
router.get('/vendor-plans', requireRoles('VENDOR_OWNER','VENDOR_STAFF','SUPER_ADMIN','ADMIN'), async(_req,res,next)=>{try{
  const r=await query(`SELECT id,code,name,monthly_price_paise,features,is_active FROM vendor_plans WHERE is_active=true ORDER BY monthly_price_paise ASC`);res.json({plans:r.rows,mode:'database'});
}catch(e){next(e)}});

router.get('/vendor-subscription', requireRoles('VENDOR_OWNER','VENDOR_STAFF','SUPER_ADMIN','ADMIN'), async(req,res,next)=>{try{
...
router.post('/vendor-subscription/change-plan', requireRoles('VENDOR_OWNER'), async(req,res,next)=>{try{
  const input=z.object({planId:z.string().uuid()}).parse(req.body);
  const vendor=await query<{id:string}>(`SELECT id FROM vendors WHERE owner_user_id=$1 LIMIT 1`,[req.authUser!.id]);if(!vendor.rows[0]){res.status(404).json({error:'VENDOR_NOT_FOUND'});return;}
  const plan=await query(`SELECT id,code,name,monthly_price_paise FROM vendor_plans WHERE id=$1 AND is_active=true`,[input.planId]);if(!plan.rows[0]){res.status(404).json({error:'PLAN_NOT_FOUND'});return;}
  const active=await query<{id:string}>(`SELECT id FROM vendor_subscriptions WHERE vendor_id=$1 AND status='ACTIVE' ORDER BY created_at DESC LIMIT 1`,[vendor.rows[0].id]);
  if(active.rows[0]){res.status(409).json({error:'ACTIVE_SUBSCRIPTION_REQUIRES_PROVIDER_CHECKOUT',subscriptionId:active.rows[0].id,plan:plan.rows[0]});return;}
...
router.post('/admin/vendor-subscriptions/:id/provider-state', requirePermission('MANAGE_SETTLEMENTS'), async(req,res,next)=>{try{
  const id=z.string().uuid().parse(req.params.id);const input=z.object({status:z.enum(['ACTIVE','PAST_DUE','CANCELLED','PENDING']),providerSubscriptionId:z.string().max(255).optional(),providerEventId:z.string().max(255).optional(),renewsAt:z.string().datetime().optional().nullable()}).parse(req.body);
  const client=await pool.connect();try{await client.query('BEGIN');
    if(input.providerEventId){const exists=await client.query(`SELECT id FROM vendor_subscription_events WHERE provider_event_id=$1`,[input.providerEventId]);if(exists.rows[0]){await client.query('COMMIT');res.json({ok:true,replayed:true});return;}}
    const r=await client.query(`UPDATE vendor_subscriptions SET status=$1,provider_subscription_id=COALESCE($2,provider_subscription_id),renews_at=$3,updated_at=NOW(),cancelled_at=CASE WHEN $1='CANCELLED' THEN NOW() ELSE cancelled_at END WHERE id=$4 RETURNING *`,[input.status,input.providerSubscriptionId??null,input.renewsAt??null,id]);if(!r.rows[0]){await client.query('ROLLBACK');res.status(404).json({error:'SUBSCRIPTION_NOT_FOUND'});return;}
    ...
    await client.query('COMMIT');await audit('VENDOR_SUBSCRIPTION_PROVIDER_STATE','vendor_subscriptions',id,req.authUser!.id,input);res.json({subscription:r.rows[0]});
  }catch(e){await client.query('ROLLBACK');throw e;}finally{client.release();}
}catch(e){next(e)}});

router.get('/admin/analytics', requirePermission('VIEW_DASHBOARD'), async(req,res,next)=>{try{
