...
router.get('/overview', async (_req,res,next)=>{ try {
  ...
  const targetVendors=Number(settings.rows.find((x:any)=>x.key==='year1_vendor_target')?.value?.value??10000);
  const density=Number(settings.rows.find((x:any)=>x.key==='minimum_orders_per_vendor_day')?.value?.value??100);
...
router.post('/launches', requirePermission('MANAGE_SYSTEM_CONFIG'), async (req,res,next)=>{ try {
  ...
  const row=r.rows[0]; await audit('MARKET_LAUNCH_UPSERTED','market_launches',row.id,req.authUser!.id,{city:input.city,state:input.state,status:input.status}); res.status(201).json({launch:row});
} catch(e){next(e);} });

const settingSchema=z.object({key:z.enum(['year1_vendor_target','minimum_orders_per_vendor_day','pilot_stage']),value:z.unknown()});
router.patch('/settings', requirePermission('MANAGE_SYSTEM_CONFIG'), async (req,res,next)=>{ try {
...
router.post('/automation/run', requirePermission('MANAGE_SYSTEM_CONFIG'), async (req,res,next)=>{try{
...
router.get('/sla', requirePermission('VIEW_ORDERS'), async (_req,res,next)=>{try{
...
router.patch('/sla/policies/:status', requirePermission('MANAGE_SYSTEM_CONFIG'), async (req,res,next)=>{try{
...
router.post('/refunds', requirePermission('APPROVE_REFUNDS'), async (req,res,next)=>{try{
...
router.get('/refunds', requirePermission('VIEW_SETTLEMENTS'), async (_req,res,next)=>{try{
...
router.get('/command-center', requirePermission('VIEW_DASHBOARD'), async (_req,res,next)=>{try{
...
router.get('/events/stream', requirePermission('VIEW_DASHBOARD'), async (req,res)=>{
 res.status(200).set({'Content-Type':'text/event-stream','Cache-Control':'no-cache','Connection':'keep-alive','X-Accel-Buffering':'no'});res.flushHeaders?.();let last='';const send=async()=>{try{const r=await query(`SELECT id,event_type,aggregate_type,aggregate_id,payload,created_at FROM integration_outbox WHERE created_at>=NOW()-INTERVAL '5 minutes' ORDER BY created_at DESC LIMIT 30`);const rows=r.rows.filter((x:any)=>x.id!==last);if(rows.length){last=rows[0].id;res.write(`event: aaspass\ndata: ${JSON.stringify(rows)}\n\n`);}else res.write(': heartbeat\\n\\n');}catch{res.write(': error\\n\\n');}};await send();const timer=setInterval(()=>void send(),3000);req.on('close',()=>clearInterval(timer));});

router.post('/reconciliation/finance', requirePermission('VIEW_SETTLEMENTS'), async (req,res,next)=>{try{const {reconcileFinance}=await import('../../services/finance/reconciliation.js');const result=await reconcileFinance(req.authUser!.id);await audit('FINANCE_RECONCILIATION_RUN','finance_reconciliation_runs',result.runId,req.authUser!.id,result);res.json(result);}catch(e){next(e);}});
router.post('/reconciliation/provider/cashfree', requirePermission('MANAGE_SYSTEM_CONFIG'), async (req,res,next)=>{try{const {reconcileCashfreePayments}=await import('../../services/finance/reconciliation.js');const result=await reconcileCashfreePayments(req.authUser!.id);await audit('PROVIDER_RECONCILIATION_RUN','provider_reconciliation_runs',result.runId,req.authUser!.id,result);res.json(result);}catch(e){next(e);}});
router.get('/reconciliation/runs', requirePermission('VIEW_SETTLEMENTS'), async (_req,res,next)=>{try{const [finance,provider]=await Promise.all([query(`SELECT * FROM finance_reconciliation_runs ORDER BY started_at DESC LIMIT 20`),query(`SELECT * FROM provider_reconciliation_runs ORDER BY started_at DESC LIMIT 20`)]);res.json({finance:finance.rows,provider:provider.rows});}catch(e){next(e);}});
...
router.patch('/alerts/:id', requirePermission('MANAGE_SYSTEM_CONFIG'), async (req,res,next)=>{try{
 const id=z.string().uuid().parse(req.params.id);const input=z.object({status:z.enum(['OPEN','ACKNOWLEDGED','RESOLVED'])}).parse(req.body);
 ...
 if(!r.rows[0]){res.status(404).json({error:'ALERT_NOT_FOUND'});return;} await audit('OPERATIONAL_ALERT_STATUS_CHANGED','operational_alerts',id,req.authUser!.id,{status:input.status});res.json({alert:r.rows[0]});
}catch(e){next(e);}});
router.post('/alerts/run', requirePermission('MANAGE_SYSTEM_CONFIG'), async (req,res,next)=>{try{const {runOperationalAlertAutomation}=await import('../../services/operations/alerts.js');const result=await runOperationalAlertAutomation();await audit('OPERATIONAL_ALERT_AUTOMATION_RUN','operational_alerts',null,req.authUser!.id,result);res.json(result);}catch(e){next(e);}});

router.get('/alerts', requirePermission('VIEW_DASHBOARD'), async (req,res,next)=>{try{const status=z.enum(['OPEN','ACKNOWLEDGED','RESOLVED']).optional().parse(req.query.status);const limit=z.coerce.number().int().min(1).max(500).default(200).parse(req.query.limit);const sql=`SELECT a.*,o.status order_status,v.name vendor_name,u.name target_user_name FROM operational_alerts a LEFT JOIN orders o ON o.id=a.order_id LEFT JOIN vendors v ON v.id=a.vendor_id LEFT JOIN users u ON u.id=a.target_user_id ${status?'WHERE a.status=$1':''} ORDER BY CASE a.severity WHEN 'CRITICAL' THEN 1 WHEN 'HIGH' THEN 2 WHEN 'WARNING' THEN 3 ELSE 4 END,a.last_detected_at DESC LIMIT $${status?2:1}`;const r=await query(sql,status?[status,limit]:[limit]);res.json({alerts:r.rows});}catch(e){next(e);}});
router.patch('/alerts/:id', requirePermission('MANAGE_SYSTEM_CONFIG'), async (req,res,next)=>{try{const id=z.string().uuid().parse(req.params.id);const input=z.object({status:z.enum(['OPEN','ACKNOWLEDGED','RESOLVED'])}).parse(req.body);const r=await query(`UPDATE operational_alerts SET status=$1,acknowledged_at=CASE WHEN $1='ACKNOWLEDGED' THEN COALESCE(acknowledged_at,NOW()) ELSE acknowledged_at END,resolved_at=CASE WHEN $1='RESOLVED' THEN NOW() ELSE NULL END,updated_at=NOW() WHERE id=$2 RETURNING *`,[input.status,id]);if(!r.rows[0]){res.status(404).json({error:'ALERT_NOT_FOUND'});return;}await audit('OPERATIONAL_ALERT_STATUS_CHANGED','operational_alerts',id,req.authUser!.id,{status:input.status});res.json({alert:r.rows[0]});}catch(e){next(e);}});
router.post('/alerts/run', requirePermission('MANAGE_SYSTEM_CONFIG'), async (req,res,next)=>{try{const {runOperationalAlertAutomation}=await import('../../services/operations/alerts.js');const result=await runOperationalAlertAutomation();await audit('OPERATIONAL_ALERT_AUTOMATION_RUN','operational_alerts',null,req.authUser!.id,result);res.json(result);}catch(e){next(e);}});
