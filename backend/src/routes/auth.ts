...
router.post(
  '/dev/login',
  rateLimit('auth-dev-login', env.AUTH_RATE_LIMIT_MAX),
  async (req, res, next) => {
    try {
       ...
       */
      if (
        (isProduction || !env.DEV_AUTH_ENABLED) &&
      ...
      const existing = await query<{
        ...
        'SELECT id,role,name FROM users WHERE phone=$1 LIMIT 1',
        [input.phone]
      );
      ...
      if (user.role === 'VENDOR_OWNER') {
        ...
        if (vendorId && plan.rows[0]?.id) {
          ...
          await query(
            `INSERT INTO vendor_kyc(vendor_id,status)
             VALUES($1,'PENDING')
      ...
      if (user.role === 'CUSTOMER') {
        const profile = await query<{ customer_code: string }>(
...
type AdminLoginRole = (typeof ADMIN_LOGIN_ROLES)[number];
...
router.post(
  '/login/request',
  rateLimit('auth-login-request', env.AUTH_RATE_LIMIT_MAX),
  async (req, res, next) => {
    try {
      const input = requestLoginOtp.parse(req.body);

      const { dbPhone, whatsappPhone } =
      ...
      if (!user) {
        res.status(401).json({
          error: 'INVALID_LOGIN',
          message:
            'The supplied account is not authorized for this application.',
          requestId: req.requestId
        });
        return;
      }
      ...
      const message =
        `Your AasPass Admin login OTP is ${otp}. ` +
...
router.post(
  '/login/verify',
  rateLimit('auth-login-verify', env.AUTH_RATE_LIMIT_MAX),
  async (req, res, next) => {
    try {
