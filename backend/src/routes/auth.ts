...
router.post('/dev/login', rateLimit('auth-dev-login', env.AUTH_RATE_LIMIT_MAX), async (req, res, next) => {
  try {
    if (isProduction || !env.DEV_AUTH_ENABLED) { res.status(404).json({ error: 'NOT_FOUND', requestId: req.requestId }); return; }
    const input = login.parse(req.body);
    if (input.otp !== env.DEV_OTP) { res.status(401).json({ error: 'INVALID_OTP', requestId: req.requestId }); return; }
    const existing = await query<{ id:string; role:(typeof ROLES)[number]; name:string }>('SELECT id,role,name FROM users WHERE phone=$1 LIMIT 1',[input.phone]);
    ...
    const result = await query<{ id: string; role: (typeof ROLES)[number]; name: string }>(
      `INSERT INTO users(phone,name,role) VALUES($1,$2,$3) ON CONFLICT(phone) DO UPDATE SET name=EXCLUDED.name,updated_at=NOW() RETURNING id,role,name`,
    ...
    if (user.role === 'VENDOR_OWNER') {
      const plan = await query<{ id: string }>("SELECT id FROM vendor_plans WHERE code='BUSINESS' LIMIT 1");
      const vendor = await query<{ id: string }>(
        "INSERT INTO vendors(owner_user_id,plan_id,name,slug,status,rating,review_count,is_open,phone) VALUES($1,$2,$3,$4,'PENDING',0,0,FALSE,$5) ON CONFLICT(owner_user_id) DO UPDATE SET name=EXCLUDED.name,updated_at=NOW() RETURNING id",
        [user.id, plan.rows[0]?.id ?? null, input.name, `aaspass-dev-store-${user.id.slice(0,8)}`, input.phone]
      );
    ...
    if (user.role === 'CUSTOMER') {
      const profile = await query<{customer_code:string}>(`SELECT customer_code FROM customer_profiles WHERE user_id=$1 LIMIT 1`, [user.id]);
      customerCode = profile.rows[0]?.customer_code;
    }

    const tokenParts = accessToken.split('.');
    const payloadPart = tokenParts[1];
    if (!payloadPart) throw new Error('Token payload missing');
    const decoded = JSON.parse(Buffer.from(payloadPart, 'base64url').toString('utf8')) as { jti: string; exp: number };
    await query(
      `INSERT INTO auth_sessions(jti,user_id,expires_at,last_seen_at,ip_address,user_agent) VALUES($1,$2,to_timestamp($3),NOW(),$4,$5) ON CONFLICT(jti) DO NOTHING`,
      [decoded.jti, user.id, decoded.exp, req.ip ?? null, req.header('user-agent') ?? null]
    );
