...
async function resolvePartnerId(userId:string):Promise<string|null>{
  ...
  const r=await query<{id:string}>('SELECT id FROM delivery_partners WHERE user_id=$1 LIMIT 1',[userId]);
  return r.rows[0]?.id??null;
}


router.get('/me', async (req,res,next)=>{try{
  const id=await resolvePartnerId(req.authUser!.id); if(!id){res.status(404).json({error:'DELIVERY_PARTNER_NOT_FOUND'});return;}
...
router.post('/status', async (req,res,next)=>{try{
  ...
  if(!id){res.status(404).json({error:'DELIVERY_PARTNER_NOT_FOUND'});return;}
...
router.post('/location', async (req,res,next)=>{try{
  const input=locationSchema.parse(req.body); const id=await resolvePartnerId(req.authUser!.id);
  if(!id){res.status(404).json({error:'DELIVERY_PARTNER_NOT_FOUND'});return;}
...
router.get('/jobs', async (req,res,next)=>{try{
  const lat=Number(req.query.latitude); const lon=Number(req.query.longitude); const radiusKm=Math.min(Math.max(Number(req.query.radiusKm??5),0.5),15);
  if(!Number.isFinite(lat)||!Number.isFinite(lon)){res.status(400).json({error:'LOCATION_REQUIRED'});return;}
  ...
  }).filter((job:any)=>job.distanceKm==null||job.distanceKm<=radiusKm);
  res.json({jobs,mode:'database'});
}catch(e){next(e)}});

router.post('/assignments/:id/accept', async (req,res,next)=>{try{
  const assignmentId=z.string().uuid().parse(req.params.id); const partnerId=await resolvePartnerId(req.authUser!.id);
  if(!partnerId){res.status(404).json({error:'DELIVERY_PARTNER_NOT_FOUND'});return;}
  ...
  try{
    ...
    if(!current.rows[0]){await client.query('ROLLBACK');res.status(404).json({error:'ASSIGNMENT_NOT_FOUND'});return;}
    if(current.rows[0].status!=='OFFERED'){await client.query('ROLLBACK');res.status(409).json({error:'ASSIGNMENT_NOT_AVAILABLE',status:current.rows[0].status});return;}
...
router.get('/assignments/:id/pickup-code', async (req,res,next)=>{try{
  const assignmentId=z.string().uuid().parse(req.params.id); const partnerId=await resolvePartnerId(req.authUser!.id);
  if(!partnerId){res.status(404).json({error:'DELIVERY_PARTNER_NOT_FOUND'});return;}
  const row=await query<{id:string;status:string}>(`SELECT id,status FROM delivery_assignments WHERE id=$1 AND delivery_partner_id=$2 LIMIT 1`,[assignmentId,partnerId]);
  if(!row.rows[0]){res.status(404).json({error:'ASSIGNMENT_NOT_FOUND'});return;}
...
router.post('/assignments/:id/verify-otp', async (req,res,next)=>{try{
  ...
  if(!partnerId){res.status(404).json({error:'DELIVERY_PARTNER_NOT_FOUND'});return;}
  const owner=await query<{id:string}>(`SELECT id FROM delivery_assignments WHERE id=$1 AND delivery_partner_id=$2 LIMIT 1`,[assignmentId,partnerId]);
  if(!owner.rows[0]){res.status(404).json({error:'ASSIGNMENT_NOT_FOUND'});return;}
  const ok=await verifyDeliveryOtp(assignmentId,stage,otp);
  if(!ok){res.status(401).json({error:'INVALID_OTP'});return;}
  res.json({verified:true,stage});
}catch(e){next(e)}});

router.post('/assignments/:id/proof', async (req,res,next)=>{try{
  const assignmentId=z.string().uuid().parse(req.params.id); const partnerId=await resolvePartnerId(req.authUser!.id);
  ...
  if(!partnerId){res.status(404).json({error:'DELIVERY_PARTNER_NOT_FOUND'});return;}
  const row=await query<{id:string;proof:any}>(`SELECT id,proof FROM delivery_assignments WHERE id=$1 AND delivery_partner_id=$2 LIMIT 1`,[assignmentId,partnerId]);
  if(!row.rows[0]){res.status(404).json({error:'ASSIGNMENT_NOT_FOUND'});return;}
...
const nextMap:Record<string,{next:string;order:string}>= {
...
router.post('/assignments/:id/status', async (req,res,next)=>{ const client=await pool.connect(); try{
  const assignmentId=z.string().uuid().parse(req.params.id); const requested=assignmentStatus.parse(req.body.status); const partnerId=await resolvePartnerId(req.authUser!.id);
  if(!partnerId){res.status(404).json({error:'DELIVERY_PARTNER_NOT_FOUND'});return;}
  const current=await client.query<{id:string;order_id:string;status:string}>(`SELECT id,order_id,status::text FROM delivery_assignments WHERE id=$1 AND delivery_partner_id=$2 FOR UPDATE`,[assignmentId,partnerId]);
  if(!current.rows[0]){res.status(404).json({error:'ASSIGNMENT_NOT_FOUND'});return;}
  const allowed=nextMap[current.rows[0].status];
  if(!allowed || allowed.next!==requested){res.status(409).json({error:'INVALID_DELIVERY_TRANSITION',from:current.rows[0].status,to:requested});return;}
  if(requested==='PICKED_UP' || requested==='DELIVERED'){
    const stage=requested==='PICKED_UP'?'PICKUP':'DROP'; const otp=z.string().regex(/^\d{6}$/).parse(req.body.otp);
    const verified=await verifyDeliveryOtp(assignmentId,stage,otp); if(!verified){res.status(401).json({error:'INVALID_OTP',stage});return;}
  }
  await client.query('BEGIN');
...
router.get('/earnings', async (req,res,next)=>{try{
  const partnerId=await resolvePartnerId(req.authUser!.id);
  if(!partnerId){res.status(404).json({error:'DELIVERY_PARTNER_NOT_FOUND'});return;}
  ...
  const total=rows.rows.reduce((a:any,b:any)=>a+Number(b.earning_paise)+Number(b.bonus_paise),0);
  const today=rows.rows.filter((r:any)=>r.delivered_at && new Date(r.delivered_at).toDateString()===new Date().toDateString()).reduce((a:any,b:any)=>a+Number(b.earning_paise)+Number(b.bonus_paise),0);
