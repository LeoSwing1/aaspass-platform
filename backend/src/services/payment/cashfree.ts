...
export async function createCashfreeOrder(input: {
  ...
  returnUrl: string;
  notifyUrl: string;
}): Promise<{ orderId: string; paymentSessionId: string; raw: unknown }> {
  ...
  const data = raw as { order_id?: string; payment_session_id?: string };
...
export async function getCashfreePayments(orderId: string): Promise<unknown> {
  if (!cashfreeConfigured()) throw new Error('CASHFREE_NOT_CONFIGURED');
  ...
  const raw = await response.json();
  if (!response.ok) throw new Error(`CASHFREE_GET_PAYMENTS_FAILED:${response.status}:${JSON.stringify(raw)}`);
  return raw;
}
...
export type ParsedCashfreePaymentEvent = {
  orderId: string;
 ...
 */
export function parseCashfreePaymentEvent(
  event: Record<string, unknown>,
): ParsedCashfreePaymentEvent | null {
  ...
  let status: ParsedCashfreePaymentEvent['status'] = 'PENDING';
