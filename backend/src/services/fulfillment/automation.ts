...
export async function runFulfillmentAutomation(): Promise<{expiredOffers:number;dispatchAttempts:number;assigned:number;failed:number;refundsSynced:number}> {
  let expiredOffers = 0;
  ...
  const expired = await query<{id:string;order_id:string;delivery_partner_id:string|null}>(`
  ...
  for (const row of expired.rows) {
    ...
    if (row.delivery_partner_id) await query(`UPDATE delivery_partners SET status='AVAILABLE',updated_at=NOW() WHERE id=$1 AND status='ASSIGNED'`,[row.delivery_partner_id]);
  ...
  for (const row of pending.rows) {
    ...
    try {
      ...
      if (result.assigned) assigned++; else failed++;
    } catch {
      failed++;
    }
  }
  const pendingRefunds=await query<{id:string}>(`SELECT id FROM refunds WHERE status IN ('PENDING','PROCESSING') AND provider='CASHFREE' AND provider_ref IS NOT NULL ORDER BY created_at ASC LIMIT 50`);
  for(const refund of pendingRefunds.rows){try{await syncProviderRefund(refund.id);refundsSynced++;}catch{}}
  return {expiredOffers,dispatchAttempts,assigned,failed,refundsSynced};
}

export async function runSlaAutomation(): Promise<{breaches:number;notifications:number}> {
  const policies = await query<{status:string;max_minutes:number;escalation_level:string}>(`SELECT status,max_minutes,escalation_level FROM order_sla_policies WHERE is_active=true`);
  let breaches=0; let notifications=0;
  for (const policy of policies.rows) {
    const rows = await query<{id:string;customer_id:string;vendor_owner_user_id:string|null;status:string;age_minutes:number}>(`
    ...
    for (const row of rows.rows) {
      breaches++;
      const upsert = await query<{id:string;last_notified_at:string|null}>(`
      ...
      if(row.vendor_owner_user_id) await queueNotification({userId:row.vendor_owner_user_id,channel:'PUSH',title,body,data:{type:'SLA_ALERT',orderId:row.id,status:row.status}});
      notifications++;
