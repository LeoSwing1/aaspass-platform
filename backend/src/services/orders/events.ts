...
const customerMessages: Record<string,[string,string]>={
...
export async function recordOrderEvent(input:{orderId:string;eventType:string;source:string;actorUserId?:string|null;payload?:Record<string,unknown>;notify?:boolean}):Promise<void>{
  ...
  if(msg) await queueNotification({userId:row.customer_id,channel:'PUSH',title:msg[0],body:`${msg[1]} Order ${String(row.id).slice(0,8)}…`,data:{type:'ORDER',orderId:row.id,status:input.eventType}});
  if(['PAID','COD_CONFIRMED'].includes(input.eventType) && row.owner_user_id) await queueNotification({userId:row.owner_user_id,channel:'PUSH',title:'New AasPass order',body:`A new order ${String(row.id).slice(0,8)}… is waiting in your store queue.`,data:{type:'VENDOR_ORDER',orderId:row.id,status:input.eventType}});
  if(input.eventType==='DELIVERY_ASSIGNED' && row.owner_user_id) await queueNotification({userId:row.owner_user_id,channel:'PUSH',title:'Delivery partner assigned',body:`A delivery partner has been assigned to order ${String(row.id).slice(0,8)}…`,data:{type:'DELIVERY_ASSIGNED',orderId:row.id,status:input.eventType}});
}
