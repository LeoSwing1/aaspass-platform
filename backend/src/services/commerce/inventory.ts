...
export async function releaseInventoryForOrder(client: PoolClient, orderId: string): Promise<number> {
  const rows = await client.query<{product_id:string;quantity:number}>(`SELECT product_id,quantity FROM inventory_reservations WHERE order_id=$1 AND status='HELD' FOR UPDATE`, [orderId]);
...
export async function commitInventoryForOrder(client: PoolClient, orderId: string): Promise<number> {
  const rows = await client.query<{product_id:string;quantity:number}>(`SELECT product_id,quantity FROM inventory_reservations WHERE order_id=$1 AND status='HELD' FOR UPDATE`, [orderId]);
  for (const row of rows.rows) {
