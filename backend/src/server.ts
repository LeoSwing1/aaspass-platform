...
app.use(express.json({ limit: '1mb', verify: (req, _res, buf) => { (req as Request & { rawBody?: string }).rawBody = buf.toString('utf8'); } }));
...
const reconciliationWorker = env.DATABASE_URL
  ? setInterval(() => { void reconcileFinance(null).catch((error) => console.error('finance reconciliation worker error', error)); if (cashfreeConfigured()) void reconcileCashfreePayments(null).catch((error) => console.error('provider reconciliation worker error', error)); }, 300000)
  : null;
...
async function shutdown(signal: string): Promise<void> {
  ...
  server.close(async () => {
    try { if (notificationWorker) clearInterval(notificationWorker); if (operationsWorker) clearInterval(operationsWorker); if (reconciliationWorker) clearInterval(reconciliationWorker); await pool?.end(); } finally { process.exit(0); }
  });
